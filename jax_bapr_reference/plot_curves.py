#!/usr/bin/env python3
"""Plot training curves for BAPR / ESCP / RE-SAC experiments.
...
COLORS = {"resac": "#4C9BE8", "escp": "#F4A261", "bapr": "#2EC4B6"}
...
def find_log(log_dir, algo, env, seed=8):
    """Find the most-recently-modified log matching this algo+env combo.

    Handles any naming convention: v3/v4/v5, _fixed, plain seeds, etc.
    Falls back to the canonical name if nothing is found.
    """
    import glob, os
    env_short = env.split('-')[0]  # 'Hopper-v2' -> 'Hopper'
    # Match: bapr_Hopper*.log, bapr_*Hopper*.log, etc.
    patterns = [
    ...
    if matches:
        # Prefer the most recently modified (i.e., the active run)
        return max(set(matches), key=os.path.getmtime)
    return os.path.join(log_dir, f"{algo}_{env}_{seed}.log")  # fallback
...
def plot(log_dir, envs, out_path, seed=8, max_iters=2000, title_extra=""):
    ...
    # Title
    algo_str = " · ".join(LABELS[a] for a in ALGOS)
    fig.text(0.5, 0.975, f"{algo_str}  —  Training Curves{title_extra}",
        ...
        for row in range(2):
            ...
            for algo in ALGOS:
                ...
                if row == 0 and len(d['iters']) > 5:
                    ...
                    xs = d['iters'][idx]
                    ax.plot(xs, sm, color=c, lw=2.0, label=lbl, alpha=0.95, zorder=3)
            ...
            if row == 0:
                ...
                # Progress badge
                n_iters = max(
                    (d['iters'][-1] if (d := all_data[(a, env)]) is not None
                     and len(d['iters']) > 0 else 0)
                    for a in ALGOS
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot BAPR/ESCP/RE-SAC training curves")
